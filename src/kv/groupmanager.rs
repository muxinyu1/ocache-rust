...
use crate::kv::{
    byteview::ByteView, dbgetter::DbGetter, error::Error, group::Group, peer::PeerPicker,
};
...
impl GroupManager {
    pub fn new(group_infos: Vec<(String, usize)>, db_getter: Arc<dyn DbGetter>) -> Self {
        ...
        for (group_name, max_bytes) in group_infos {
            map.insert(
                group_name.clone(),
                Group::new(group_name, db_getter.clone(), max_bytes),
            );
        }
        return GroupManager {
            mtx: Arc::new(Mutex::new(map)),
        };
    }
    ...
    pub fn register_peer_for_group(
        &self,
